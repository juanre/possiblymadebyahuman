// Generated from Unicode 17.0.0 CaseFolding.txt (2025-07-30).
// Source: https://www.unicode.org/Public/17.0.0/ucd/CaseFolding.txt
//
// PMBAH canon-letters/0.1 uses Unicode full case folding with status C + F
// mappings and intentionally excludes Turkic-only status T mappings.

const FULL_CASE_FOLDING: ReadonlyMap<number, readonly number[]> = new Map([
  [0x0041, [0x0061]],
  [0x0042, [0x0062]],
  [0x0043, [0x0063]],
  [0x0044, [0x0064]],
...
export function unicodeFullCasefold(input: string): string {
  const output: string[] = [];
  for (const codepoint of input) {
    const mapping = FULL_CASE_FOLDING.get(codepoint.codePointAt(0) ?? 0);
    output.push(mapping === undefined ? codepoint : String.fromCodePoint(...mapping));
  }
  return output.join("");
}
