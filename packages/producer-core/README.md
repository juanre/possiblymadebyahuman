...
- **`resume`** — explicitly reattaches an unfinished session while preserving its clock, history and observation credentials. The next real edit marks unknown continuity with `pos: null`, except for an empty draft resumed in an empty field. No synthetic edit is inserted.
