...
- Public records are content-blind by default: they store edit structure, metadata, statistics, and analyzer facts, not plaintext writing.
- Plaintext is allowed only in local replay flows and test fixtures needed to verify deterministic behavior.
...
1. `packages/format` defines the shared contract and future canonicalize/hash/replay/verify APIs.
...
Implement event/manifest types, canonical JSON serialization, BLAKE3 `b3:` hashing, event hash-chain verification, deterministic replay with Unicode codepoint offsets, and conformance vectors.
...
Build the public record page, content-blind replay scrubber, quick stats, signal cards, and verification panel.
