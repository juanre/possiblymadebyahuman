...
The replacement release candidate tag is v0.3.0-rc.2. It restores the established release workflow: tag releases publish the `latest` image used by this deployment. The first candidate, v0.3.0-rc.1, did not update `latest` because of an unintended workflow change. The extension package reports 0.3.0 (Chrome's numeric version requirement), while the GitHub release is explicitly a prerelease. Creating the image does not deploy it. The owner deploys through their existing Render workflow.
