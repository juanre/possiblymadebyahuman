...
`possiblymadebyahuman` records the *editing process* that produced a piece of writing and presents it as an inspectable, hash-addressed process record. A reader can inspect operation shape and timing without seeing the words. The name's hedge is deliberate and load-bearing: "possibly" is the scope boundary, and the boundary is what makes the gesture worth anything.

### 1.1 The thesis: costly signaling, not proof

The value of this system is **not** that it proves authorship. It can't, and we say so. The value is that producing and sharing a record is *costly* — you ran the capture, you're handing someone an inspectable process record, you're inviting scrutiny instead of asserting a verdict. **Caring enough to show your work is the message.** Like a handwritten thank-you note, it proves nothing about sincerity yet is unfakeable-in-spirit, because the people who don't mean it don't bother.
...
```
  ...
  Layer 3  ANALYZERS          timing · edit-topology · paste-ratio · pause/position coupling · …  (plugins, §6)
              │  Signals (§6.2)
              ▼
  Layer 4  PRESENTATION       hash-addressed page: process timeline + signals as facts (§7)
```
...
Enumerated, extensible, and central to accurate reporting about input fidelity:
...
A producer **must not** label something `typing` it cannot prove was typing. `unknown` is the correct fallback and must be available. A producer that can only ever emit `unknown` (e.g. a Docs-history importer) declares this via capabilities (§4.2) so analyzers do not penalize a coarse source for looking suspicious.
...
- **paste-ratio** — fraction of observed insertion volume arriving via `source: paste`/`drop`/large atomic inserts.
...
1. **Process timeline (the hero).** Shows the event log as operation shape over time: insert/delete positions, sizes, sources, timing, large atomic inserts, and long pauses. It does not reconstruct or render document text. The timeline is the thing that gives a skeptic concrete process facts to inspect — make it primary, not the signals.
...
3. **Verification panel.** The full `record_hash`; a one-click "re-verify chain" that recomputes it in the browser; producer id/version/capabilities; which attestations are present (e.g. "timeline server-attested" vs "client-claimed time only"); analyzer ids/versions.
...
|---|---|
| Copy-paste of AI text into the editor | **Yes** — appears as a large atomic insert with `source: paste`; visible in the process timeline and edit-topology. |
...
| Fully synthetic log (a script emits a plausible internally-consistent mutation stream, never touching a keyboard) | **No, and v0 does not try.** The hash chain can still pass — the log is internally consistent, just fabricated. This is acceptable: it costs real effort to fake a gesture that carries no verdict and grants no authority, so the incentive is largely absent. Defenses (streaming server timestamp, entropy checks, client attestation) are **deferred** (§1.4, §4.3) until a proof-grade audience needs them. |
...
What we *do* defend, cheaply, with the hash chain: a shared record can't be tampered with after the fact or swapped behind its URL without detection. Everything beyond it is a later, optional, proof-grade tier — not the thing we are shipping or the thing we are.
...
- **Process-length vectors**: event logs → expected observed final length, or `null` when process measurements are unknown. Catches codepoint/UTF-16/byte length bugs without text fixtures.
- **Capability accuracy checks**: a producer declaring `source_attribution` must never emit `unknown` for events it could attribute; importers must declare absence.
...
A producer is "conformant" iff it passes canonicalization + hash-chain + process-length + capability-accuracy checks.
...
- **No OS-level keystroke capture.** A producer that owns its text buffer (browser field, emacs buffer, editor extension) can emit accurate mutation positions and lengths; an OS keylogger observes a motor stream that does *not* describe the final document in someone else's app, dropping us from "process record" to "correlation," which is both forgeable and unreliable. Producers must own a buffer.
...
- **Mutation** — one buffer change: `{seq, t, op, pos, del_len, ins_len, source}`. Unknown `pos`, `del_len`, or `ins_len` values are explicit JSON `null`.
