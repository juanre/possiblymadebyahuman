...
- The session is saved to `pmbah-state-directory` (default `~/.emacs.d/pmbah/`) by a background worker. Editing queues numeric events; the worker acknowledges them only after syncing the journal and its metadata. Checkpoint results are also saved in the background. Normal close, exit, pause, rename, and signing wait for pending saves. Reopening an opted-in file resumes its session: earlier events are kept and new event times continue from the original start, so leaving for hours, days or months shows up as a pause inside one record. An append-only journal holds the numeric public events; a separate small state file holds the session id, start time, format version, journal cursor, and checkpoint token — never document text — and is readable only by you.
...
- A checkout or release directory containing both `pmbah-mode.el` and `scripts/event-journal.mjs`.
...
`pmbah-state-directory` (default `~/.emacs.d/pmbah/`) holds owner-only session metadata and an append-only numeric-event journal. Existing saved sessions migrate automatically before their old metadata is replaced. Point it elsewhere if your Emacs directory is synced between machines.
