...
3. **Process-length vectors** — given a vector event log, the producer computes the same observed process length and follows the same rules for emitting explicit `null` in `pos`, `del_len`, or `ins_len` when a value cannot be derived content-opaquely.
