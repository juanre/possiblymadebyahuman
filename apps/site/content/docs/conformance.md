...
1. **Canonicalization vectors** — given an event, the producer canonicalises it byte-identically to the format package (sorted keys, no whitespace, fixed numeric formatting).
...
3. **Process-length vectors** — given a vector event log, the producer computes the same `event_count` and `duration_ms` and follows the same rules for emitting explicit `null` in `pos`, `del_len`, or `ins_len` when a value cannot be derived content-opaquely.
...
- `unknown` is what you use when you cannot reliably attribute the input. It is a first-class value, not a failure state, and it is what keeps the format precise.
