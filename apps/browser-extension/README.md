...
- **Empty field, no resumable session** → fresh session, badge reads `PMBAH · writing record ↗`.
- **Empty field, resumable session matches** → the existing session is resumed, badge reads `PMBAH · writing record (resumed) ↗`.
...
- **Non-empty field, no resumable session** → INELIGIBLE. Badge reads `existing text — start in an empty field`. To start a session in this field the user must clear the field and focus it again, or open a fresh one; an ineligible field is re-evaluated on every focus.
...
1. Focus a textarea or plain text input. The badge appears: `PMBAH · writing record ↗`, `PMBAH · writing record (resumed) ↗`, or `existing text — start in an empty field`.
...
6. Open any page with a textarea and start typing. The “PMBAH · writing record ↗” control appears at the top right of the field. Click it to open the session panel. Reload pages that were already open when you installed or reloaded the extension. The control follows editor movement and hides when the field is hidden or removed. Chrome 127+ supports opening the panel this way; if unavailable, the control directs you to the toolbar menu.
...
- **Pre-existing content INELIGIBLE.** Open a page where a textarea already has some text (e.g. a draft restored by the site itself). Focus it. The badge should read `existing text — start in an empty field`. Clear the field, click elsewhere and focus it again; the badge should switch to `PMBAH · writing record ↗`.
- **Editing after signing.** Sign a field, then paste the link into it. The badge must read `PMBAH · writing record (continues a signed record) ↗`, not an error; the popup must still list the signed session with its link, plus a new one-event session marked as continuing it. Signing the continuation must produce a record whose page says "Continues from" the first.
