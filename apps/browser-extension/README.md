...
3. Write in that editor. Other fields remain inactive. The side panel follows the exact editor in the active tab of its own browser window. “This editor” highlights that draft; an untracked field is identified explicitly. Other drafts remain available below. Rename a draft privately to distinguish it from other fields; that name does not become its public label. The extension records edit times, positions, lengths, and sources where measurable; it does not retain your words.
4. Choose **Finish & get link**, review the page URL, title and label, and choose whether readers may check a copy of its text. The review shows **Selected text** or **Whole field** separately. Changing the selection or editing after review requires a new confirmation; changing focus never changes which draft is being finished. **How the text check works** explains the limitations before publication. Confirming freezes the draft and signs its finish time before uploading.
