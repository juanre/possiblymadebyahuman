...
type Routing = { routes: Route[]; snapshots: Record<string, Snapshot>; selected?: string; last_start_error?: string };
...
const grants = new Map<string, { tab: number; frame: number; document?: string; share?: string }>();
...
async function startEditor(tab: number, frame: number, mode: "context" | "focused", share?: string): Promise<BackgroundResponse> {
  await ready;
  ...
  grants.set(activation_id, { tab, frame, document: frameInfo.documentId, share });
  starting.add(`${tab}:${frame}`);
  try {
    const result = await chrome.tabs.sendMessage(tab, { kind: "start_editor", target: mode, activation_id, ...(share ? { share_session_id: share } : {}) }, { frameId: frame, ...(frameInfo.documentId ? { documentId: frameInfo.documentId } : {}) }) as BackgroundResponse;
  ...
  finally { grants.delete(activation_id); starting.delete(`${tab}:${frame}`); }
}
async function focusedEditor(share?: string): Promise<BackgroundResponse> {
  ...
  return startEditor(tab.id, candidates[0]!, "focused", share);
}
async function freeze(session: string, bind = false): Promise<Snapshot> {
  ...
  const task = (async (): Promise<Snapshot> => {
    ...
    const shared = allRoutes.length > 1;
...
async function routeMessage(message: ContentToBackground, sender: Sender): Promise<BackgroundResponse> {
  ...
  if (sender.tab && !isOwnUI(sender)) {
    ...
    if (message.kind === "register_field") {
      ...
      if (!grant || grant.tab !== tab || grant.frame !== frame || grant.document !== sender.documentId || grant.share !== message.share_session_id) return { kind: "error", reason: "explicit_start_required" };
  ...
  if (message.kind === "start_focused_editor") return focusedEditor(message.share_session_id);
